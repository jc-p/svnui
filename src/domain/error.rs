...
pub fn explain(stderr: &str) -> Option<(String, String)> {
    ...
    let (summary, action) = match code.as_deref() {
        ...
        // ⚠️ 必须放在 E170013 分支**之前**：
        // 按子树授权的仓库里，svn 会同时报
        //   E170013 Unable to connect ...（结果）
        //   E175013 Access to '...' forbidden（原因）
        // 匹配到前者会建议"检查网络/VPN" —— 方向完全错了，
        // 用户会去查 VPN，而真正的原因是这个账号看不到该路径。
        Some("E175013") => (
            "无权访问该路径",
            concat!(
                "这个账号看不到该路径 —— 常见于按子树授权的仓库：\n",
                "能进子目录，但看不了仓库根。\n",
                "仓库面板默认从工作副本所在目录打开；向上退到无权限的一层会报这个错。\n",
                "确实需要更上层的话，找管理员开通权限。",
            ),
        ),
...
fn extract_code(stderr: &str) -> Option<String> {
    ...
    const PRIORITY: &[&str] = &[
        ...
        "E215004", // 认证失败
        "E215001", // 无权限
        "E175013", // 无权访问该路径（E170013 常常是它的结果，必须排在前面）
        "E155004", // 工作副本锁
        "E155009",
        "E155007", // 不是工作副本
        "E160013", // 路径不存在
